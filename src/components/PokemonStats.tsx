import type { Pokemon, PokemonStat, Type as PokemonTypeData } from 'pokenode-ts';
...
const calculateTypeEffectiveness = (typesData: PokemonTypeData[]): { [key: string]: number } => {
  ...
  for (const type of typesData) {
    ...
    relations.double_damage_from.forEach(t => effectivenessMap[t.name] *= 2);
    relations.half_damage_from.forEach(t => effectivenessMap[t.name] *= 0.5);
...
export const StatsScene: React.FC<{ pokemon: Pokemon; typesData: PokemonTypeData[]; speciesHexColor: string; isLandscape: boolean }> = ({ pokemon, typesData, speciesHexColor, isLandscape }) => {
  ...
  const totalStats = useMemo(() => pokemon.stats.reduce((sum, stat) => sum + stat.base_stat, 0), [pokemon.stats]);
  const typeEffectiveness = useMemo(() => calculateTypeEffectiveness(typesData), [typesData]);
    ...
    <ScrollView
      ...
      <View style={[cardStyle, shadowStyle]}>
        ...
        <View style={styles.statRow}>
          <Text style={[styles.statLabel, { color: theme.textMuted, fontWeight: 'bold' }]}>Total</Text>
          <Text style={[styles.statValue, { color: theme.text, fontWeight: 'bold' }]}>{totalStats}</Text>
      ...
      <View style={[cardStyle, shadowStyle]}>
        ...
        {defenses.weak.length > 0 && (
          <View style={styles.defenseSection}>
            <Text style={[styles.defenseLabel, textStyle]}>Weak Against (x{Math.max(...defenses.weak.map(t => typeEffectiveness[t])).toFixed(1)} damage):</Text>
            <View style={styles.typeBadgeContainer}>
              {defenses.weak.map(type => <TypeBadge key={type} typeName={type} />)}
            </View>
          </View>
        )}
        {defenses.resistant.length > 0 && (
          <View style={styles.defenseSection}>
            <Text style={[styles.defenseLabel, textStyle]}>Resistant To (x{Math.min(...defenses.resistant.map(t => typeEffectiveness[t])).toFixed(1)} damage):</Text>
            <View style={styles.typeBadgeContainer}>
              {defenses.resistant.map(type => <TypeBadge key={type} typeName={type} />)}
            </View>
          </View>
        )}
        {defenses.immune.length > 0 && (
          <View style={styles.defenseSection}>
            <Text style={[styles.defenseLabel, textStyle]}>Immune To (x0):</Text>
            <View style={styles.typeBadgeContainer}>
              {defenses.immune.map(type => <TypeBadge key={type} typeName={type} />)}
            </View>
          </View>
        )}
        {defenses.weak.length === 0 && defenses.resistant.length === 0 && defenses.immune.length === 0 && (
          <Text style={textStyle}>Normal effectiveness against all types.</Text>
        )}
      </View>
    </ScrollView>
  );
