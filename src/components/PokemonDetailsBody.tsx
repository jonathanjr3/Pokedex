import type { Pokemon, PokemonSpecies, Type as PokemonTypeData } from 'pokenode-ts';
...
const PokemonDetailsBody: React.FC<Props> = ({ pokemon, species, typesData, speciesHexColor, isLandscape }) => {
  ...
  const renderScene = ({ route }: SceneRendererProps & { route: TabRoute }) => {
    switch (route.key) {
      case 'about':
        return species ? <AboutScene pokemon={pokemon} species={species} isLandscape={isLandscape} /> : null;
      case 'stats':
        return typesData.length > 0 ? <StatsScene pokemon={pokemon} typesData={typesData} speciesHexColor={speciesHexColor} isLandscape={isLandscape} /> : null;
      default:
        return null;
    }
  };
  ...
  const renderTabBar = (props: SceneRendererProps & { navigationState: NavigationState<TabRoute> }) => (
    <TabBar<TabRoute>
      ...
      pressColor={isDarkMode ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)'}
      scrollEnabled={false}
    />
  );
