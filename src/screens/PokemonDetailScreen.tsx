...
import { getBackgroundColorFromSpecies, getPokemonColorHex } from '../styles/PokemonSpeciesColors';
...
const PokemonDetailScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const speciesHexColor = getBackgroundColorFromSpecies(species?.color?.name, theme);
  const statBarSpeciesColor = getPokemonColorHex(species?.color?.name) ?? theme.primary;
      ...
      <SafeAreaView style={[styles.centerContainer, backgroundStyle]}>
        <View style={[containerStyle, { justifyContent: 'center', alignItems: 'center' }]}>
          ...
          <Text style={{ color: theme.text, marginTop: 10 }}>
            {isLoadingDetails ? 'Loading Pokémon details...' : errorDetails ?? 'Could not load Pokémon.'}
          </Text>
        </View>
      </SafeAreaView>
    );
        ...
        {isLandscape ? (
          <View style={styles.landscapeContainer}>
            <View style={styles.landscapeLeftColumn}>
              <PokemonHeader pokemon={pokemon} isLandscape={isLandscape} speciesHexColor={speciesHexColor} />
            </View>
